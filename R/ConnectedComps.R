...
wrapper.compute.cc <- function(obj.se) {
    ...
    stopifnot(
        "adjacencyMatrix" %in% colnames(SummarizedExperiment::rowData(obj.se))
        )
...
#'
#' @param clickFunction A JavaScript code to show an information when a point 
#' is clicked
#' @export
#'
#' @import highcharter
...
plotJitter_hc <- function(ll.cc,
                          df.tooltips = NULL,
                          clickFunction = NULL) {
    n.prot <- unlist(lapply(ll.cc, function(x) {
        length(x$proteins)
    }))
    n.pept <- unlist(lapply(ll.cc, function(x) {
        length(x$peptides)
    }))
    ...
    if (!is.null(df.tooltips)) {
        ...
        stopifnot(
            length(dim(df.tooltips)) == 2 && length(colnames(df.tooltips)) > 0
            )
...
Extract_CC <- function(ll.cc, type) {
    ...
    ll.prot <- lapply(ll.cc, function(x) {
        length(x$proteins)
    })
    ll.pept <- lapply(ll.cc, function(x) {
        length(x$peptides)
    })
