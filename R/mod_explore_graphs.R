...
mod_explore_graphs_server <- function(id, 
  ...
  moduleServer(id, function(input, output, session){
    ...
    BuildOne2OneTab <- reactive({
      ll.cc()
      table <- do.call(rbind,
                       lapply(Extract_CC(ll.cc(), 'OneOne'),
                              function(x){data.frame(rbind(x))}))
      
      if(is.null(table))
        table <- data.frame(proteins=character(), 
                            peptides = character()
                            )
      ...
      dt <- datatable( data,
                       ...
                       options = list(initComplete = .initComplete(),
                                      ...
                                      columnDefs = list(
                                        list(
                                          targets = c(
                                            (((ncol(data)-offset)/2)+1):(ncol(data)-offset)
                                          ), 
                                          visible = FALSE)
                                      )
                       )) %>%
        DT::formatStyle(
          ...
          backgroundColor = DT::styleEqual(c("POV", "MEC"), c('orange', 'blue')))
    ...
    BuildOne2MultiTab <- reactive({
      ll.cc()
      table <- do.call(rbind,
                       lapply(Extract_CC(ll.cc(), 'OneMulti'),
                              function(x){
                                data.frame(rbind(x), 
    ...
    output$graphicalView_ui <- renderHighchart({
      ...
      clickFun <-
        JS(paste0("function(event) {Shiny.onInputChange('",
                  ns("eventPointClicked"),
                  "', [this.index]+'_'+ [this.series.name]);}"))
      
      plotCC <-  plotJitter_hc(ll.cc = Extract_CC(ll.cc(), 'MultiAny'),
                               df.tooltips = tooltip,
    ...
    output$tabularView_ui <- DT::renderDT({
      ...
      df <- do.call(rbind,lapply(Extract_CC(ll.cc(), 'MultiAny'),
                                 function(x){
                                   data.frame(rbind(x),
                                              nPep = length(x$peptides),
    ...
    observeEvent(req(input$searchCC), {
      ...
      shinyjs::toggle('graphicalView_ui', condition = input$searchCC == 'graphical')
      shinyjs::toggle('tabularView_ui', condition = input$searchCC=='tabular')
    })
    ...
    output$visNet_CC <- renderVisNetwork({
      ...
      rv.cc$selectedCCgraph <- convertCC2graph(cc = local[[rv.cc$selectedCC]], 
    ...
    BuildMulti2AnyTab <- reactive({
      ll.cc()
      table <- do.call(rbind,
                       lapply(Extract_CC(ll.cc(), 'MultiAny'),
                              function(x){
