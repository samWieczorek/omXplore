...
mvPerLinesHisto <- function(se,
    ...
    df <- data.frame(
        y = nb.na2barplot,
        y_percent = round(100 * nb.na2barplot / dim(data)[1], digits = 2)
    )
    ...
    h1 <- highchart() %>%
        ...
        hc_tooltip(
            enabled = TRUE,
            headerFormat = "",
            pointFormat = paste0("{point.y} lines<br>({point.y_percent}% 
                of all lines)")
        )
...
mvPerLinesHistoPerCondition <- function(data,
    ...
    h1 <- highchart() %>%
        ...
        hc_legend(enabled = FALSE) %>%
        hc_xAxis(categories = row.names(m), 
            title = list(text = "#[NA values] per line (condition-wise)")) %>%
